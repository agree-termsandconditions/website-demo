...
function init() {
	...
	// 🎉 Birthday surprise auto-launch
	setTimeout(() => {
	toggleSound(true);            // Enable sound
	store.setState({ finale: true }); // Start finale mode
	}, 2000); // Start surprise 2 seconds after load
...
if (IS_HEADER) {
	// init(); // Disabled for manual launch
} else {
	setLoadingStatus('Lighting Crackers');
	setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    ['.long-exposure-label', '.settings-btn', '.quality-ui-label', '.pause-btn', '.sound-btn'].forEach(sel => {
        document.querySelector(sel)?.remove();
    });
    
    // Set quality to high and enable sound
    store.setState({ config: { ...store.state.config, quality: "3", soundEnabled: true, volume: "1" } });
    toggleSound(true);
    setVolume(1);
    ...
    volumeSlider.style.height = '150px';
    volumeSlider.style.width = '10px';
    volumeSlider.style.transform = 'rotate(-90deg)';
    volumeSlider.style.transformOrigin = 'bottom left';
    ...
    volumeSlider.addEventListener('input', (e) => {
        ...
        store.setState({ config: { ...store.state.config, volume } });
        setVolume(volume);
    });
});
